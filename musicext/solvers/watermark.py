...
def get_encodec_audio_effect(encodec_cfg: DictConfig, sr: int) -> tp.Dict:
    ...
    is put here instead of in `musicext.utils.audio_effects` because
    it depends on the package `musicext.solvers`, which is one layer
    higher than `musicext.utils`, so we avoid the circle dependency
    from any solvers using `musicext.utils.audio_effects` to do the
    augmentation
    """
